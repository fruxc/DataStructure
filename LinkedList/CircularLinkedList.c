...
void deleteE()
{
        ...
        {
            q = list;
            while(q->next != list)		//while to point q pointer at the very last location of linked list, we're running it till end.
			{
				r = q;			//r will store previous pointed out location of linked list
				q=q->next;		//to increment q
			}

		r->next=list;			//setting previous location's next pointer to NULL since it's very last element so it should point to NULL.
        if(q->next == q)
		{
			list = NULL;
		}
...
void deleteP()
{
        ...
        while(q->data!=data1 && q->next != list)	//to check given input is present in the linked list or not.
		{	
			r = q;                  //r will store previous pointed out location of linked list
            q=q->next;              //to increment q
		}
...
{	
	...
	if(list == NULL)		//to check whether it is empty or not
	{
		...
		list=list->next;
		while(q->next!=list)		//running a while till it reaches its very last element
		{
			r = q->next;	//r to point out next of q
			while(r->next!=list)	//running till r the end of the linked list
			{
